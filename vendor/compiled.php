...
class Guard implements GuardContract {
	...
	protected function fireLoginEvent($user, $remember = false) {
		if (isset ( $this->events )) {
			$this->events->fire ( 'auth.login', array (
					$user,
					$remember 
			) );
		}
	...
	public function logout() {
		...
		if (isset ( $this->events )) {
			$this->events->fire ( 'auth.logout', array (
					$user 
			) );
		}
		$this->user = null;
...
class Application extends Container implements ApplicationContract, HttpKernelInterface {
	...
	public function bootstrapWith(array $bootstrappers) {
		foreach ( $bootstrappers as $bootstrapper ) {
			$this ['events']->fire ( 'bootstrapping: ' . $bootstrapper, array (
					$this 
			) );
			$this->make ( $bootstrapper )->bootstrap ( $this );
			$this ['events']->fire ( 'bootstrapped: ' . $bootstrapper, array (
					$this 
			) );
		}
	...
	protected function markAsRegistered($provider) {
		$this ['events']->fire ( $class = get_class ( $provider ), array (
				$provider 
		) );
	...
	public function setLocale($locale) {
		...
		$this ['events']->fire ( 'locale.changed', array (
				$locale 
		) );
	}
...
trait AuthenticatesAndRegistersUsers {
	...
	public function postLogin(Request $request) {
		...
		return redirect ( $this->loginPath () )->withInput ( $request->only ( 'email', 'remember' ) )->withErrors ( array (
	...
	public function postReset(Request $request) {
		...
		switch ($response) {
			case PasswordBroker::PASSWORD_RESET :
				return redirect ( $this->redirectPath () );
			default :
				return redirect ()->back ()->withInput ( $request->only ( 'email' ) )->withErrors ( array (
						'email' => trans ( $response ) 
				) );
		}
	}
...
class VerifyCsrfToken implements Middleware {
	...
	protected function isReading($request) {
		return in_array ( $request->method (), array (
				'HEAD',
				'GET',
				'OPTIONS' 
		) );
...
class FormRequest extends Request implements ValidatesWhenResolved {
	...
	protected function getValidatorInstance() {
		...
		return $factory->make ( $this->all (), $this->container->call ( array (
				$this,
				'rules' 
...
class BcryptHasher implements HasherContract {
	...
	public function needsRehash($hashedValue, array $options = array()) {
		$cost = isset ( $options ['rounds'] ) ? $options ['rounds'] : $this->rounds;
		return password_needs_rehash ( $hashedValue, PASSWORD_BCRYPT, array (
				'cost' => $cost 
		) );
	}
...
class Str {
	...
	public static function parseCallback($callback, $default) {
		return static::contains ( $callback, '@' ) ? explode ( '@', $callback, 2 ) : array (
				$callback,
				$default 
		);
	}
...
trait RouteDependencyResolverTrait {
	protected function callWithDependencies($instance, $method) {
		return call_user_func_array ( array (
				$instance,
				$method 
		), $this->resolveClassMethodDependencies ( array (), $instance, $method ) );
	}
	...
	public function resolveMethodDependencies(array $parameters, ReflectionFunctionAbstract $reflector) {
		foreach ( $reflector->getParameters () as $key => $parameter ) {
			...
			if ($class && ! $this->alreadyInParameters ( $class->name, $parameters )) {
				array_splice ( $parameters, $key, 0, array (
...
class Router implements RegistrarContract {
	...
	public static function mergeGroup($new, $old) {
		...
		return array_merge_recursive ( array_except ( $old, array (
				'namespace',
				'prefix',
				'where' 
		) ), $new );
	...
	public function dispatchToRoute(Request $request) {
		...
		$this->events->fire ( 'router.matched', array (
				$route,
				$request 
		) );
	...
	protected function callFilter($filter, $request, $response = null) {
		return $this->events->until ( 'router.' . $filter, array (
				$request,
				$response 
		) );
	}
...
class Route {
	...
	protected function parseWhere($name, $expression) {
		return is_array ( $name ) ? $name : array (
				$name => $expression 
		);
	}
...
class RouteCollection implements Countable, IteratorAggregate {
	...
	protected function checkForAlternateVerbs($request) {
		$methods = array_diff ( Router::$verbs, array (
				$request->getMethod () 
		) );
		$others = array ();
...
abstract class Controller {
	...
	protected function registerInstanceFilter($filter) {
		$this->getRouter ()->filter ( $filter, array (
				$this,
				substr ( $filter, 1 ) 
		) );
		return $filter;
...
class UrlGenerator implements UrlGeneratorContract {
	...
	protected function addPortToDomain($domain) {
		if (in_array ( $this->request->getPort (), array (
				'80',
				'443' 
		) )) {
			return $domain;
...
class Dispatcher implements DispatcherContract {
	...
	protected function createQueuedHandlerCallable($class, $method) {
		return function () use($class, $method) {
			...
			if (method_exists ( $class, 'queue' )) {
				...
				$this->resolveQueue ()->push ( 'Illuminate\\Events\\CallQueuedHandler@call', array (
						'class' => $class,
						'method' => $method,
	...
	protected function callQueueMethodOnHandler($class, $method, $arguments) {
		...
		$handler->queue ( $this->resolveQueue (), 'Illuminate\\Events\\CallQueuedHandler@call', array (
				'class' => $class,
				'method' => $method,
				'data' => serialize ( $arguments ) 
		) );
...
abstract class Model implements ArrayAccess, Arrayable, Jsonable, JsonSerializable, QueueableEntity, UrlRoutable {
	...
	public function push() {
		...
		foreach ( $this->relations as $models ) {
			$models = $models instanceof Collection ? $models->all () : array (
					$models 
			);
...
class DatabaseManager implements ConnectionResolverInterface {
	...
	protected function parseConnectionName($name) {
		...
		return Str::endsWith ( $name, array (
				'::read',
				'::write' 
		) ) ? explode ( '::', $name, 2 ) : array (
				$name,
				null 
		);
	}
...
class ConnectionFactory {
	...
	protected function mergeReadWriteConfig(array $config, array $merge) {
		return array_except ( array_merge ( $config, $merge ), array (
				'read',
				'write' 
		) );
	}
...
class StartSession implements TerminableMiddleware {
	...
	protected function sessionIsPersistent(array $config = null) {
		...
		return ! in_array ( $config ['driver'], array (
				null,
				'array' 
		) );
	}
...
class Store implements SessionInterface {
	...
	protected function loadSession() {
		...
		foreach ( array_merge ( $this->bags, array (
				$this->metaBag 
		) ) as $bag ) {
	...
	public function flash($key, $value) {
		...
		$this->removeFromOldFlashData ( array (
				$key 
		) );
	}
...
class CookieJar implements JarContract {
	...
	public function queue() {
		if (head ( func_get_args () ) instanceof Cookie) {
			...
			$cookie = call_user_func_array ( array (
					$this,
					'make' 
			), func_get_args () );
		}
	...
	public function setDefaultPathAndDomain($path, $domain) {
		list ( $this->path, $this->domain ) = array (
				$path,
				$domain 
		);
		return $this;
...
class Factory implements FactoryContract {
	...
	public function callComposer(View $view) {
		$this->events->fire ( 'composing: ' . $view->getName (), array (
				$view 
		) );
	}
	public function callCreator(View $view) {
		$this->events->fire ( 'creating: ' . $view->getName (), array (
				$view 
		) );
	}
	...
	public function addExtension($extension, $engine, $resolver = null) {
		...
		$this->extensions = array_merge ( array (
				$extension => $engine 
		), $this->extensions );
	}
	public function getExtensions() {
...
class MessageBag implements Arrayable, Countable, Jsonable, JsonSerializable, MessageBagContract, MessageProvider {
	...
	protected function transform($messages, $format, $messageKey) {
		...
		foreach ( $messages as &$message ) {
			...
			$message = str_replace ( $replace, array (
					$message,
					$messageKey 
			), $format );
		}
...
final class SecureRandom implements SecureRandomInterface {
	...
	private function updateSeed() {
		if (! $this->seedUpdated && $this->seedLastUpdatedAt < time () - mt_rand ( 1, 10 )) {
			file_put_contents ( $this->seedFile, json_encode ( array (
					$this->seed,
					microtime ( true ) 
			) ) );
		}
