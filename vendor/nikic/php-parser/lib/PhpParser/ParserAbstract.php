...
abstract class ParserAbstract {
	...
	/**
	 * @var array Map of states to a displacement into the $action table.
	 * The corresponding action for this
	 * state/symbol pair is $action[$actionBase[$state] + $symbol]. If $actionBase[$state] is 0, the
	 * action is defaulted, i.e. $actionDefault[$state] should be used instead.
	 */
	protected $actionBase;
	/**
	 * @var array Table of actions.
	 * Indexed according to $actionBase comment.
	 */
	protected $action;
	/**
	 * @var array Table indexed analogously to $action.
	 * If $actionCheck[$actionBase[$state] + $symbol] != $symbol
	...
	/**
	 * @var array Map of non-terminals to a displacement into the $goto table.
	 * The corresponding goto state for this
	 * non-terminal/state pair is $goto[$gotoBase[$nonTerminal] + $state] (unless defaulted)
	 */
	protected $gotoBase;
	/**
	 * @var array Table of states to goto after reduction.
	 * Indexed according to $gotoBase comment.
	 */
	protected $goto;
	/**
	 * @var array Table indexed analogously to $goto.
	 * If $gotoCheck[$gotoBase[$nonTerminal] + $state] != $nonTerminal
	...
	/**
	 * @var array Map of rules to the non-terminal on their left-hand side, i.e.
	 * the non-terminal to use for
		...
		for(;;) {
			...
			for(;;) {
				if ($rule === 0) {
					...
					try {
						$this->{'reduceRule' . $rule} ( $attributeStack [$this->stackPos - $this->ruleToLength [$rule]] + $endAttributes );
					} catch ( Error $e ) {
					...
					throw new Error ( 'Syntax error, unexpected ' . $this->symbolToName [$symbol] . $expectedString, $startAttributes ['startLine'] );
				}
			}
		}
	}
	...
	protected function traceNewState($state, $symbol) {
		echo '% State ' . $state . ', Lookahead ' . ($symbol == self::SYMBOL_NONE ? '--none--' : $this->symbolToName [$symbol]) . "\n";
	}
