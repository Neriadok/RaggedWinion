...
class AuthorizationCheckerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testVoteAuthenticatesTokenIfNecessary() {
		...
		$this->authenticationManager->expects ( $this->once () )->method ( 'authenticate' )->with ( $this->equalTo ( $token ) )->will ( $this->returnValue ( $newToken ) );
		...
		$this->accessDecisionManager->expects ( $this->once () )->method ( 'decide' )->with ( $this->callback ( $tokenComparison ) )->will ( $this->returnValue ( true ) );
	...
	public function testIsGranted($decide) {
		...
		$this->accessDecisionManager->expects ( $this->once () )->method ( 'decide' )->will ( $this->returnValue ( $decide ) );
		$this->tokenStorage->setToken ( $token );
