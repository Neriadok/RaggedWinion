...
class RememberMeAuthenticationProviderTest extends \PHPUnit_Framework_TestCase {
	...
	public function testAuthenticateWhenPreChecksFails() {
		...
		$userChecker->expects ( $this->once () )->method ( 'checkPreAuth' )->will ( $this->throwException ( new DisabledException () ) );
	...
	public function testAuthenticate() {
		...
		$user->expects ( $this->exactly ( 2 ) )->method ( 'getRoles' )->will ( $this->returnValue ( array (
				'ROLE_FOO' 
		) ) );
	...
	protected function getSupportedToken($user = null, $key = 'test') {
		...
		$token = $this->getMock ( 'Symfony\Component\Security\Core\Authentication\Token\RememberMeToken', array (
				'getProviderKey' 
		), array (
				$user,
				'foo',
