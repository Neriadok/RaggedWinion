...
class AbstractTokenTest extends \PHPUnit_Framework_TestCase {
	...
	public function testSerializeParent() {
		...
		$token = new ConcreteToken ( $user, array (
				'ROLE_FOO' 
		) );
		
		$parentToken = new ConcreteToken ( $user, array (
				new SwitchUserRole ( 'ROLE_PREVIOUS', $token ) 
		) );
		$uToken = unserialize ( serialize ( $parentToken ) );
		
		$this->assertEquals ( current ( $parentToken->getRoles () )->getSource ()->getUser (), current ( $uToken->getRoles () )->getSource ()->getUser () );
	}
	...
	protected function getToken(array $roles = array()) {
		return $this->getMockForAbstractClass ( 'Symfony\Component\Security\Core\Authentication\Token\AbstractToken', array (
				$roles 
		) );
	}
}
