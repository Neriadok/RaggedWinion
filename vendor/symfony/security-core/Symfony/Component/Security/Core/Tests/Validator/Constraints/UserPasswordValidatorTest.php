...
abstract class UserPasswordValidatorTest extends AbstractConstraintValidatorTest {
	...
	public function testPasswordIsValid() {
		...
		$this->encoder->expects ( $this->once () )->method ( 'isPasswordValid' )->with ( static::PASSWORD, 'secret', static::SALT )->will ( $this->returnValue ( true ) );
	...
	protected function createUser() {
		...
		$mock->expects ( $this->any () )->method ( 'getPassword' )->will ( $this->returnValue ( static::PASSWORD ) );
		
		$mock->expects ( $this->any () )->method ( 'getSalt' )->will ( $this->returnValue ( static::SALT ) );
	...
	protected function createEncoderFactory($encoder = null) {
		...
		$mock->expects ( $this->any () )->method ( 'getEncoder' )->will ( $this->returnValue ( $encoder ) );
	...
	protected function createSecurityContext($user = null) {
		...
		$mock->expects ( $this->any () )->method ( 'getToken' )->will ( $this->returnValue ( $token ) );
