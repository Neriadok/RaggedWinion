...
class AuthenticationProviderManagerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function getAuthenticationProvider($supports, $token = null, $exception = null) {
		...
		$provider->expects ( $this->once () )->method ( 'supports' )->will ( $this->returnValue ( $supports ) );
		
		if (null !== $token) {
			$provider->expects ( $this->once () )->method ( 'authenticate' )->will ( $this->returnValue ( $token ) );
		} elseif (null !== $exception) {
			$provider->expects ( $this->once () )->method ( 'authenticate' )->will ( $this->throwException ( $this->getMock ( $exception, null, array (), '' ) ) );
		}
