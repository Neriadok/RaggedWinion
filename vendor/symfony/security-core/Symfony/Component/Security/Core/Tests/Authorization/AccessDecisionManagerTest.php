...
class AccessDecisionManagerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testSetUnsupportedStrategy() {
		new AccessDecisionManager ( array (
	...
	public function testStrategies($strategy, $voters, $allowIfAllAbstainDecisions, $allowIfEqualGrantedDeniedDecisions, $expected) {
		...
		$this->assertSame ( $expected, $manager->decide ( $token, array (
				'ROLE_FOO' 
		) ) );
	}
	...
	protected function getVoterFor2Roles($token, $vote1, $vote2) {
		...
		$voter->expects ( $this->exactly ( 2 ) )->method ( 'vote' )->will ( $this->returnValueMap ( array (
				array (
						$token,
						null,
						array (
	...
	protected function getVoter($vote) {
		$voter = $this->getMock ( 'Symfony\Component\Security\Core\Authorization\Voter\VoterInterface' );
		$voter->expects ( $this->any () )->method ( 'vote' )->will ( $this->returnValue ( $vote ) );
		
		return $voter;
	}
	protected function getVoterSupportsClass($ret) {
		$voter = $this->getMock ( 'Symfony\Component\Security\Core\Authorization\Voter\VoterInterface' );
		$voter->expects ( $this->any () )->method ( 'supportsClass' )->will ( $this->returnValue ( $ret ) );
		
		return $voter;
	}
	protected function getVoterSupportsAttribute($ret) {
		$voter = $this->getMock ( 'Symfony\Component\Security\Core\Authorization\Voter\VoterInterface' );
		$voter->expects ( $this->any () )->method ( 'supportsAttribute' )->will ( $this->returnValue ( $ret ) );
