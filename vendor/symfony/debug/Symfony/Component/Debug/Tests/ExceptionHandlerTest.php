...
class ExceptionHandlerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testHeaders() {
		...
		$response = $handler->createResponse ( new MethodNotAllowedHttpException ( array (
				'POST' 
		) ) );
	...
	public function testHandle() {
		...
		if (class_exists ( 'Symfony\Component\HttpFoundation\Response' )) {
			...
			$handler->expects ( $this->exactly ( 2 ) )->method ( 'createResponse' )->will ( $this->returnValue ( new Response () ) );
		} else {
			$handler = $this->getMock ( 'Symfony\Component\Debug\ExceptionHandler', array (
					'sendPhpResponse' 
			) );
	...
	public function testHandleOutOfMemoryException() {
		...
		if (class_exists ( 'Symfony\Component\HttpFoundation\Response' )) {
			...
			$handler->expects ( $this->once () )->method ( 'createResponse' )->will ( $this->returnValue ( new Response () ) );
		} else {
			$handler = $this->getMock ( 'Symfony\Component\Debug\ExceptionHandler', array (
					'sendPhpResponse' 
			) );
