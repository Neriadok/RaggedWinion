...
abstract class Swift_Transport_AbstractSmtpEventSupportTest extends Swift_Transport_AbstractSmtpTest {
	...
	public function testSendingDispatchesBeforeSendEvent() {
		...
		$message->shouldReceive ( 'getFrom' )->zeroOrMoreTimes ()->andReturn ( array (
	...
	public function testSendEventCapturesFailures() {
		...
		$dispatcher->shouldReceive ( 'dispatchEvent' )->zeroOrMoreTimes ();
		$evt->shouldReceive ( 'bubbleCancelled' )->zeroOrMoreTimes ()->andReturn ( false );
		$evt->shouldReceive ( 'setFailedRecipients' )->once ()->with ( array (
	...
	public function testCancellingBubbleBeforeTransportStoppedStopsEvent() {
		...
		$hasRun = false;
		$dispatcher->shouldReceive ( 'createTransportChangeEvent' )->atLeast ()->once ()->with ( $smtp )->andReturn ( $evt );
		$dispatcher->shouldReceive ( 'dispatchEvent' )->once ()->with ( $evt, 'beforeTransportStopped' )->andReturnUsing ( function () use(&$hasRun) {
			$hasRun = true;
		} );
		$dispatcher->shouldReceive ( 'dispatchEvent' )->zeroOrMoreTimes ();
	...
	public function testResponseEventsAreGenerated() {
		...
		$dispatcher->shouldReceive ( 'createResponseEvent' )->atLeast ()->once ()->with ( $smtp, \Mockery::any (), \Mockery::any () )->andReturn ( $evt );
	...
	public function testCommandEventsAreGenerated() {
		...
		$dispatcher->shouldReceive ( 'createCommandEvent' )->once ()->with ( $smtp, \Mockery::any (), \Mockery::any () )->andReturn ( $evt );
	...
	public function testExceptionsCauseExceptionEvents() {
		...
		$dispatcher->shouldReceive ( 'createTransportExceptionEvent' )->zeroOrMoreTimes ()->with ( $smtp, \Mockery::any () )->andReturn ( $evt );
	...
	public function testExceptionBubblesCanBeCancelled() {
		...
		$buf->shouldReceive ( 'readLine' )->atLeast ()->once ()->andReturn ( "503 I'm sleepy, go away!\r\n" );
		$dispatcher->shouldReceive ( 'createTransportExceptionEvent' )->twice ()->with ( $smtp, \Mockery::any () )->andReturn ( $evt );
