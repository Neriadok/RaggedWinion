...
class Swift_Transport_FailoverTransportTest extends \SwiftMailerTestCase {
	public function testFirstTransportIsUsed() {
		...
		$t1->shouldReceive ( 'send' )->twice ()->with ( \Mockery::anyOf ( $message1, $message2 ), \Mockery::any () )->andReturnUsing ( function () use(&$connectionState) {
			if ($connectionState) {
				return 1;
			}
		} );
	...
	public function testZeroIsReturnedIfTransportReturnsZero() {
		...
		$testCase = $this;
		$t1->shouldReceive ( 'send' )->once ()->with ( $message, \Mockery::any () )->andReturnUsing ( function () use(&$connectionState, $testCase) {
			if (! $connectionState) {
				$testCase->fail ();
			}
	...
	public function testTransportsWhichThrowExceptionsAreNotRetried() {
		...
		$t2->shouldReceive ( 'send' )->times ( 4 )->with ( \Mockery::anyOf ( $message1, $message2, $message3, $message4 ), \Mockery::any () )->andReturnUsing ( function () use(&$connectionState2, $e) {
			if ($connectionState2) {
				return 1;
			}
		} );
	...
	public function testRestartingTransportRestartsDeadDelegates() {
		...
		$t1->shouldReceive ( 'send' )->once ()->with ( $message2, \Mockery::any () )->andReturnUsing ( function () use(&$connectionState1) {
			if ($connectionState1) {
				return 10;
			}
		} );
